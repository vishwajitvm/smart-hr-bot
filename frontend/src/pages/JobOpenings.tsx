...
export default function JobOpenings() {
    ...
    const loadJob = async () => {
      try {
        ...
        setJob((prev) => ({
          ...
          // cached: data.cached ?? prev.cached,
          cached:
            typeof data.cached === "boolean" ? data.cached : null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await Swal.fire({
      title: id ? "Confirm Update" : "Confirm Save",
      text: id
        ? "Are you sure you want to update this job?"
        : "Are you sure you want to save this job?",
      icon: "question",
      showCancelButton: true,
      confirmButtonText: id ? "Yes, Update it" : "Yes, Save it",
    ...
    try {
      ...
      if (id) {
        // Update mode
        res = await updateJob(id, payload);
        console.log("Job updated:", res.data);
        await Swal.fire("Success", "Job updated successfully!", "success");
        navigate("/view-job-openings");
      } else {
        // Create mode
        res = await createJob(payload);
        console.log("Job saved:", res.data);
        await Swal.fire("Success", "Job saved successfully!", "success");
        navigate("/view-job-openings");
      ...
      Swal.fire(
        "Error",
        `Failed to ${id ? "update" : "save"} job. Please try again.`,
        "error"
      );
    } finally {
      setSaving(false);
  ...
  const handleAIClick = async () => {
    if (!job.title) {
      Swal.fire("Warning", "Please enter a Job Title first!", "warning");
      return;
    }
    ...
    try {
      ...
      setJob((prev) => ({
        ...
        cached:
          typeof res.data.cached === "boolean" ? res.data.cached : null,
        token: res.data.token,
      }));
    } catch (err) {
      console.error("AI generation failed:", err);
      Swal.fire("Error", "Failed to generate job details. Please try again.", "error");
    } finally {
      setLoadingAI(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="job-form">
            ...
            <div className="form-actions">
              <button type="submit" className="button is-primary" disabled={saving}>
                {saving ? (id ? "Updating..." : "Saving...") : id ? "Update Job" : "Save Job"}
              </button>
            </div>
          </form>
        )}
